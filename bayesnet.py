...
class CustomerBayesNet(BayesNet):
    ...
    def add_first_layer(e1, w1, e2, w2, e3, w3, dt):
        ...
        return [mensions, positiveness, percentage, overall_rank, reach_rank, rank_per_mill, views_rank]
    ...
    def create_conditional_probs(l):
        ...
        return [rank, reviews, website, overall]

    def bake_network(self, e1, w1, e2, w2, e3, w3, dt):
        nodes1 = self.add_first_layer(e1, w1, e2, w2, e3, w3, dt)
        nodes2 = self.create_conditional_probs(nodes1)
        n = nodes1 + nodes2

        s1 = State(n[0], name="mensions")
        s2 = State(n[1], name="positiveness")
        s3 = State(n[2], name="percentage")
        s4 = State(n[3], name="overall_rank")
        s5 = State(n[4], name="reach_rank")
        s6 = State(n[5], name="rank_per_mill")
        s7 = State(n[6], name="views_rank")
        s8 = State(n[9], name="rank")
        s9 = State(n[10], name="reviews")
        s10 = State(n[11], name="website")
        s11 = State(n[13], name="Overall")

        self.network.add_states(s1, s2, s3, s4, s5, s6, s7, s8, s9, s10, s11)
        self.network.add_edge(s1, s9)
        self.network.add_edge(s2, s9)
        self.network.add_edge(s3, s9)

        self.network.add_edge(s4, s8)
        self.network.add_edge(s5, s8)
        self.network.add_edge(s6, s8)
        self.network.add_edge(s7, s10)
        self.network.add_edge(s8, s10)

        self.network.add_edge(s9, s11)
        self.network.add_edge(s10, s11)
